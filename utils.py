...
def draw_dotted_line(frame, start_point, end_point, line_color=(0, 255, 0), dot_gap=10):
    """
    Draw a dotted line between two points on the frame.
    """
    x1, y1 = start_point
    x2, y2 = end_point
    dist = int(((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5)

    for i in range(0, dist, dot_gap):
        r = i / dist
        x = int(x1 + r * (x2 - x1))
        y = int(y1 + r * (y2 - y1))
        cv2.circle(frame, (x, y), 2, line_color, -1)
